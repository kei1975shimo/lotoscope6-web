...
## 有料機能
カバラ数秘術・タロットは生成・プレビュー両方で403にします。ストア購入検証は未接続です。TEST_PREMIUM_ACCESS は TESTING=True のテスト専用で、本番では無効です。クライアントのCookie・フォーム・ヘッダーで有料機能を解除する経路はありません。公開設定で TESTING=True を使用しないでください。
