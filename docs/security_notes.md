...
## 有料機能の一時開放
運営者の要望により `PREMIUM_PREVIEW_ENABLED` の既定値を1にし、本番・ローカルとも3占術を無料開放しています。0にして再起動すると生成・プレビューの両方で有料占術を403に戻します。利用者からのフォームやCookieでこの設定は変えられません。
ストア購入検証は未接続です。TEST_PREMIUM_ACCESSはTESTING=Trueの場合だけ有効なテスト用設定で、購入権限ではありません。公開設定でTESTING=Trueを使用しないでください。
